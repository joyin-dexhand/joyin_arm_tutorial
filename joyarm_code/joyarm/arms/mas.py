"""``Mas`` 基类 —— 多轴系统（Multi-Axis System，不含末端执行器）。

``Mas`` 持有 pinocchio 模型 ``model`` + 数据 ``data`` + 限位；本体通信后端
``backend_mas`` 由子类绑 ``backend_mas_cls``，构造时按 ``configs/*.yaml`` 的
``backend_mas`` 段实例化。

``connected=False``（默认，离线）时：
- 计算类方法（fkine/jac/...）不依赖真机，无硬件也能跑
- 执行类方法（``get_state``/``command``）``raise RuntimeError``；``connect()`` 后可用

注：含末端执行器的完整臂见 :class:`joyarm.arms.arm.Arm`（``Arm(Mas)`` 组合一个 ``End``）。
"""
...
try:  # pinocchio 为重依赖，惰性导入：缺失时仅 Mas 实例化报错
...
class Mas:
    """多轴系统基类（本体：pin model/data + 限位 + backend_mas + 运动学算法门面）。
    ...
    :param name: 名称。
    :param config: 型号 YAML 配置字典（含 ``backend_mas`` 等段）；缺省无后端。
    """

    # ---- 型号绑定：子类绑多轴本体后端**类**，构造时按 config 实例化 ----
    ...
    def __init__(
        ...
        if pin is None:
            raise ImportError(
                "Mas 需要 pinocchio 才能加载 URDF 与计算运动学。请安装：\n"
        ...
        self.joint_limits_soft: JointLimits = self._build_soft_limits(
            self.joint_limits, margin=(config or {}).get("joint_limits_soft_margin", 0.05)
        )
        ...
        # ---- 多轴本体后端：子类绑 backend_mas_cls，按 config['backend_mas'] 实例化 ----
        self.backend_mas: Optional[BackendMas] = (
            self.backend_mas_cls(**(config or {}).get("backend_mas", {}))
            if self.backend_mas_cls is not None
            else None
        )

    # ----------------------------------------------------------
    # 真机连接（connect 后才可执行 get_state/command）
    # ----------------------------------------------------------
    def connect(self) -> None:
        """连接本体真机（委托 ``backend_mas.connect()``）。"""
    ...
    # ----------------------------------------------------------
    # 执行类方法（依赖 backend_mas；未连接 raise）
    # ----------------------------------------------------------
