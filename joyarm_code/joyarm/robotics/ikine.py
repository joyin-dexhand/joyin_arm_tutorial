...
def ikine(
    ...
    **kwargs,
    ...
    :param q0: ``(n,)`` 初值；缺省用 ``mas.q_neutral``。
    :param frame: 末端帧名/索引；缺省为 ``mas.ee_frame_name``。
...
def ikine_constrained(
    ...
    **kwargs,
    ...
    :return: :class:`~joyarm.utils.types.IKResult`（解满足 ``mas.joint_limits``）。
    """
