...
- **组合根 `JoyArm`**：完整机械臂 = arm 本体 + end 执行器。持有一个**整机通信后端** `backend`（本体+末端一体，yaml `backend.name` 选型构建）与六个**策略成员**（`_fkine_solver` / `_ikine_solver` / `_jacobian_solver` / `_dynamics_solver` / `_traj_planner` / `_controller`）；公开门面（`arm.fkine()` / `arm.plan_joint()` / `arm.play()` / `arm.end_open()`…）全部委托私有成员，`connect()` 后才可执行硬件操作。
