...
│   │   │   └── planning.py          #       规划算法（关节/笛卡尔全部规划方法纯函数；规划器导入委托）
...
│   └── test_logger_perf.py          #   logger.warning 性能基准（100 次总耗时：默认/级别过滤/自定义 Handler）
...
`configs/<model>.yaml` 四段：`basic`（基本信息 + robot_model URDF 索引）/ `joyarm`（设备模型 + 软限位 margin）/ `robotics`（六域算法选型）/ `backend`（通信层）。**命名链**：工厂入参 = 文件名 = `basic.name` 字段，任一环节不符即软失败返回 `None`（约束4）；backend 与机械臂型号 1:1（`backend_dm`）：

```yaml
basic: {name, robot, ee_frame}
joyarm: {arm_mdh_and_limits, T_linkn_end, joint_soft_margins, end_soft_margins, tcp_limits, q_zero, q_home, q_neutral}   # margin 四键 {q_upper, q_lower, dq, tau} 绝对余量（rad/rad/s/N·m，标量或 n/n_end 元列表）；MDH/T_linkn_end 为教学数据（约束8）；workspace_box 兼容 (2,3)/(3,2)
robotics: {六域契约规格}   # 六域选型（注册名 / {name, **参数} / 规格列表）
backend: {name: backend_dm, arm: {channel, protocol, baud_rate, control_rate, joints}, end: {同构, joints}}   # end.joints 四限位键 q_min/q_max/dq_max/tau_max 为 ABC 契约
```

运行期可设参数走 `arm.set_config(path, value)` 白名单（`joyarm.joint_soft_margins` / `joyarm.end_soft_margins`），**不回写 yaml**（文件为单一事实来源，重启以 yaml 为准）。
...
| `robotics/control/` | `Controller(ABC)`（`step` 模板 + `_compute` 内核）+ AutoController/ForceController 桩 | Ch6/9 | 🟡 |
...
JoyArm.set_config(path, value)                              # 运行期白名单（joyarm.joint_soft_margins / end_soft_margins）✅
...
# 成员字典
JoyArm.set_solver(domain, name) · list_solvers(domain)   # 运行期切换/查询 ✅
...
JoyArm.get_arm_state() → ArmState（fkine 已注册时填 tcp.pose）· set_arm_command(mode, q/dq/tau/kp/kd, joint=None)（校验后委托后端；限位守卫在 Backend 基类模板） · read/write_param_{arm,end}
JoyArm.end_open/end_close/end_zero(joint=None) · set_end_position / set_end_force / get_end_state   # 末端守卫同在后端模板（力度按 ±tau_max 数值裁剪）
...
planning.py：cubic_q(q0,q1,s) / cubic_traj(q0,q1,t,rate)→(ts,q,dq)   # 规划算法纯函数（规划器导入委托、外部复用，如 JoyArm.move_j）✅
AutoTrajPlanner（关节/位姿 × 单值/序列 四情形）/ ForceTrajPlanner（阻抗/导纳/力位混合）   # 注释桩 🟡
Controller(ctrl_hz)：step(arm, frame, state=None)→_compute→set_arm_command（指令原样下发，限位守卫归后端基类）   # 模板已实现 ✅
